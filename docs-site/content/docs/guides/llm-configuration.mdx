...
current run, exposes only the tools passed to that run, auto-approves those
allowlisted runtime tools, asks Codex to use a read-only sandbox with other
approvals disabled, and disables Codex web search.
