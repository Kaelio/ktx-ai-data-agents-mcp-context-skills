...
| `rateLimit.maxWaitMs` | `int > 0` | unset | Optional maximum wait before the run fails instead of continuing to pause. |
| `rateLimit.retry.maxAttempts` | `int > 0` | `6` | Maximum retry attempts for opaque rate-limit responses that don't include a reset time or retry-after value. |
