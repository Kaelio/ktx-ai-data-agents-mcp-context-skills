...
accepts `codex`, `default`, or a model ID your Codex account exposes — run
`codex` to see the models available to your login — such as `gpt-5.5`. Note
that `*-codex` API-billing model IDs (for example `gpt-5.3-codex`) are not
available to ChatGPT-subscription logins.
