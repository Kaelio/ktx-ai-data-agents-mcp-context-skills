...
connections:
  - id: pg_books
    driver: postgres
    url: "postgres://user:pass@localhost:5432/books"
  - id: sqlite_reviews
    driver: sqlite
...
Inside a federated query, every table is referenced with a three-part name:
`connectionId.schema.table`. The connection's `id` field in `ktx.yaml` becomes
the catalog name inside DuckDB.
...
- `sqlite_reviews.reviews` — the `reviews` table in the sqlite connection
  (SQLite has no schema layer, so the two-part form is used)
