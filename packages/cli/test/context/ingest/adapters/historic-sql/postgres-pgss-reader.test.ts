...
describe('PostgresPgssReader aggregate path', () => {
  ...
  it('aggregates pg_stat_statements rows by queryid and query', async () => {
    ...
    for await (const row of reader.fetchAggregated(
      ...
      { dialect: 'postgres', minExecutions: 5, enabledTables: [], enabledSchemas: [], modeledTableCatalog: [], filters: { dropTrivialProbes: true }, redactionPatterns: [], staleArchiveAfterDays: 90 },
    )) {
      rows.push(row);
    }
