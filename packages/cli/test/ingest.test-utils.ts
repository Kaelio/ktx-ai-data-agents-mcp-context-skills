...
import { KtxYamlMetabaseSourceStateReader, LocalMetabaseDiscoveryCache } from '../src/context/ingest/adapters/metabase/local-source-state-store.js';
import { MetabaseSourceAdapter } from '../src/context/ingest/adapters/metabase/metabase.adapter.js';
import { getLocalIngestStatus, type LocalIngestResult, type RunLocalIngestOptions } from '../src/context/ingest/local-ingest.js';
...
import type { LookerMappingClient, LookerTableIdentifierParser } from '../src/context/ingest/adapters/looker/mapping.js';
import type { LookerRuntimeClient } from '../src/context/ingest/adapters/looker/fetch.js';
import type { MemoryFlowEventSink } from '../src/context/ingest/memory-flow/types.js';
import type { MetabaseCard, MetabaseCardSummary, MetabaseClientFactory, MetabaseRuntimeClient } from '../src/context/ingest/adapters/metabase/client-port.js';
import type { SqliteBundleIngestStore } from '../src/context/ingest/sqlite-bundle-ingest-store.js';
...
import { runKtxIngest } from '../src/ingest.js';
...
export async function localBundleStore(projectDir: string, ids: [string, string]): Promise<SqliteBundleIngestStore> {
  const { SqliteBundleIngestStore } = await import('../src/context/ingest/sqlite-bundle-ingest-store.js');;
  const project = await loadKtxProject({ projectDir });
